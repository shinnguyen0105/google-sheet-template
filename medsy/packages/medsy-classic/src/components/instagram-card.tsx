...
import { useInstagramUser } from 'helpers/use-instagram-user';
...
const InstagramPost: React.FC<InstagramPostProps> = ({ postUrl }) => {
  const { post, isLoading: postLoading } = useInstagramPost(postUrl);
  const { user, isLoading: userLoading } = useInstagramUser(postUrl);
  if (postLoading || userLoading) return null;
  return (
    <a
      href={postUrl}
    ...
    >
      ...
      <div className={InstagramItemAuthorWrapper}>
        ...
        <div className={InstagramItemAuthorInfoWrapper}>
          <div className={InstagramItemAuthorFullName}>{user?.full_name}</div>
          <div className={InstagramItemAuthorName}>{post?.author_name}</div>
        </div>
      </div>
    </a>
  );
